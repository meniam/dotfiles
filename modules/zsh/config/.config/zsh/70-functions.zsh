...
# List SSH hosts from ~/.ssh/config and its Include'd config.local and
# config.d/*.conf, skipping wildcard patterns such as "Host *". A "Host" line
# with several names prints as "main-name (alias, alias)"; connecting strips
# the parenthesized part and uses main-name.
_ssh_fzf_hosts() {
  local -a ssh_config_files
  ssh_config_files=(~/.ssh/config ~/.ssh/config.local(N) ~/.ssh/config.d/*.conf(N))
...
# The connectable host name from a picker line, stripping a trailing
# "(alias, alias)" group.
_ssh_fzf_host_name() { print -r -- "${1%% \(*}"; }

# True when the command line is the picker trigger below: "ss" alone, or
# "ss " followed by a query to prefill fzf's search with (e.g. "ss db"),
# chosen so it does not collide with typing an actual "ssh ..." command. Sets
# $_ssh_fzf_query to the part after "ss " on match.
_ssh_fzf_trigger() {
  (( $+commands[fzf] )) || return 1
  case "$BUFFER" in
    ss) _ssh_fzf_query="" ;;
    "ss "*) _ssh_fzf_query="${BUFFER#ss }" ;;
    *) return 1 ;;
  esac
}

# Fuzzy-pick an SSH host and connect. Type "ss" then Tab or Enter to open the
# picker, optionally with a search query after "ss " (e.g. "ss db"); anything
# else on Tab falls through to fzf's own Tab completion (fzf-completion, bound
# in completion.zsh), and anything else on Enter runs normally.
ssh-fzf-connect() {
  local _ssh_fzf_query
  if _ssh_fzf_trigger; then
    local host
    host="$(_ssh_fzf_hosts | command fzf --height=40% --reverse --prompt='ssh> ' --query="$_ssh_fzf_query")"

    if [[ -n "$host" ]]; then
      BUFFER="ssh $(_ssh_fzf_host_name "$host")"
      CURSOR=${#BUFFER}
      zle accept-line
    else
      zle redisplay
...
zle -N ssh-fzf-connect
bindkey '^I' ssh-fzf-connect
...
zle -N ssh-fzf-accept-line
bindkey '^M' ssh-fzf-accept-line
